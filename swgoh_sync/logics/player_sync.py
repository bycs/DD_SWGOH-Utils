...
def get_data_player(json_player):
    ...
    data.loc[:, "gp_chars":"gp_total"] = data.loc[:, "gp_chars":"gp_total"].astype(
        "int32"
    )
    return data
...
def get_units_player(json_player):
    ...
    units.loc[:, "unit.data.rarity":"unit.data.relic_tier"] = units.loc[
        :, "unit.data.rarity":"unit.data.relic_tier"
    ].astype("int8")
    units.loc[:, "unit.data.power":"unit.data.stats.28"] = units.loc[
        :, "unit.data.power":"unit.data.stats.28"
    ].astype("int32")
    units.loc[:, "unit.data.stats.5":"unit.data.stats.6"] = units.loc[
        :, "unit.data.stats.5":"unit.data.stats.6"
    ].astype("int16")
    units.loc[:, "unit.data.stats.16":"unit.data.stats.18"] = units.loc[
        :, "unit.data.stats.16":"unit.data.stats.18"
    ].astype("float16")
    units.loc[:, "unit.data.combat_type"] = units.loc[
        :, "unit.data.combat_type"
    ].astype("int8")
    units.set_axis(
        [
